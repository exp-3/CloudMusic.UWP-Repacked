...
5. 在开始菜单中找到 PowerShell，以**管理员身份**运行，执行以下命令安装 appx 软件包：

   ```
   Add-AppxPackage <路径>
   ```

   其中 `<路径>` 是您下载的 appx 文件的完整路径。`Add-AppxPackage` 不严格要求大小写。
...
4. 在开始菜单中找到 PowerShell，以**管理员身份**运行，执行以下命令安装 appx 软件包：
...
### 1. 安装 appx 时「无法安装相关项」与「解析应用包时出错」

...
1. 下载 [证书文件](tools/ca.cer) 与合适的 `appx` 软件包。
2. 右键点击 `ca.cer` 证书文件，选择 `安装证书`。
3. 选择 `本地计算机`，下一步（需要管理员权限）。
4. 选择 `将所有的证书都放入下列存储`，点击 `浏览`，选择 `受信任人`，确定，一路点击 `下一步` ，继续完成证书导入。

#### 方法二：一键安装证书

1. 下载 [证书安装脚本](tools/一键安装根证书.bat) 与 [证书文件](tools/ca.cer)，以及合适的 `appx` 软件包。
2. 右键或长按选择以管理员身份打开 `一键安装根证书.bat`，根据提示操作即可。
3. 当显示内容包含 `证书 "MyCA Root" 添加到存储` 或者 `证书 "MyCA Root" 已经在存储中` 说明安装成功。

## 安装软件
1. 下载合适的 `appx` 软件包，在开始菜单里面找到PowerShell，以管理员身份运行，执行以下命令：
   ```
   Add-AppxPackage <路径>
   ```
   其中 `<路径>` 是您下载的 `appx` 文件包的完整路径。你也可以直接双击打开安装，但大概率会失败。
2. 打开软件。如有问题，请查阅下方的 [已知问题及解决方法](#已知问题及解决方法) 。
...
#### 3. 安装 appx 时"无法安装相关项"与"解析应用包时出错"问题
...
但是，UWP客户端属于官方出品的旧版本，**似乎还不会被强风控**，算是能凑合用吧🥰。

&nbsp;

#### 6. Windows on ARM 更新 24H2+ 后无法安装和使用

此问题只影响了非常罕见的ARM64电脑用户。普通的x86/x64设备不受影响。<br>
当初UWP客户端只提供了32位的ARM版本，是为Windows Phone手机准备的。后来采用高通骁龙/苹果M处理器的电脑也算搭了个便车。<br>
